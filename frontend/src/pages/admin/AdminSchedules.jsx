...
export default function AdminSchedules() {
 ...
 <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
  {(() => {
    ...
    const grouped = doctorSchedules.reduce((acc, s) => {
      const dId = s.doctor?.doctor_id || s.doctor_id;
      if (!acc[dId]) acc[dId] = { doctor: s.doctor, schedules: [] };
      acc[dId].schedules.push(s);
      return acc;
    }, {});
    ...
    return Object.values(grouped).map(group => (
      <div key={group.doctor?.doctor_id} className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col hover:border-teal-200 dark:hover:border-teal-900 transition-all">
        <div className="flex gap-4 items-center mb-4 pb-4 border-b border-slate-50 dark:border-slate-800/50">
          <div className="h-12 w-12 rounded-2xl bg-teal-50 dark:bg-teal-900/30 flex items-center justify-center text-teal-600 text-xl shadow-inner shrink-0">
            <FaUserMd/>
          </div>
          <div>
            <h4 className="font-black text-slate-800 dark:text-white leading-tight">Dr. {group.doctor?.first_name} {group.doctor?.last_name}</h4>
            <p className="text-xs font-bold text-slate-400 mt-1">{group.schedules.length} Assigned {group.schedules.length === 1 ? 'Day' : 'Days'}</p>
          </div>
        </div>
        
        <div className="space-y-2">
          {group.schedules.sort((a, b) => DAYS.indexOf(a.day_of_week) - DAYS.indexOf(b.day_of_week)).map(s => (
            <button 
              key={s.schedule_id}
              onClick={() => setModal({ type: 'doctor', mode: 'edit', data: s })}
              className="w-full flex items-center justify-between p-3 hover:bg-slate-50 dark:hover:bg-slate-800/50 rounded-2xl transition-colors border border-transparent hover:border-slate-100 dark:hover:border-slate-700 text-left group/item"
            >
              <div>
                <p className="text-sm font-bold text-teal-600 uppercase tracking-widest">{s.day_of_week}s</p>
                <div className="flex items-center gap-2 mt-1 text-xs text-slate-500 font-medium">
                  <div className="w-1.5 h-1.5 rounded-full bg-teal-500"/> 
                  {formatTime(s.start_time)} - {formatTime(s.end_time)}
                  <span className="text-[9px] font-black text-slate-400 uppercase ml-1 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full">{s.slot_limit} Slots</span>
                </div>
              </div>
              <div className="flex flex-col items-end gap-1">
                <Badge variant={s.schedule_status === 'Active' ? 'success' : 'neutral'}>{s.schedule_status}</Badge>
                <span className="text-[10px] font-bold text-blue-500 opacity-0 group-hover/item:opacity-100 transition-opacity">
                  Click to Edit
                </span>
              </div>
            </button>
 ...
 Half Day: {formatTime12Hour(modal.data.start_time)} - {formatTime12Hour(modal.data.end_time)}
 </span>
 ) : (
 ...
 {formErrors.general && (
 <p className="mt-4 text-center text-xs font-black text-rose-500 uppercase tracking-widest animate-bounce"> {formErrors.general}</p>
 )}
 </Modal>
 )}
