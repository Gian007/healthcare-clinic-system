...
export default function DoctorAppointments(){
 ...
 <table className="w-full text-sm min-w-[900px]"><thead><tr className="text-left text-slate-500 border-b dark:border-slate-800"><th className="py-3">Patient</th><th>Service</th><th>Date</th><th>Time</th><th>Status</th><th>Notes</th><th>Actions</th></tr></thead><tbody>{filtered.map(r=><tr key={r.appointment_id} className="border-b dark:border-slate-800"><td className="py-4 font-semibold">{r.patient?.first_name} {r.patient?.last_name}</td><td>{r.service?.service_name}</td><td>{r.appointment_date}</td><td>{r.start_time}</td><td><Badge tone={r.booking_status==='Pending'?'yellow':r.booking_status==='Completed'?'green':'blue'}>{r.booking_status}</Badge></td><td className="text-xs text-slate-500 max-w-[150px] truncate">{r.completion_note || '-'}</td><td className="flex gap-2 py-3"><button onClick={()=>handleStatusUpdate(r.appointment_id,'Confirmed')} title="Called" className="text-teal-600 hover:text-teal-800 bg-teal-50 p-2 rounded-lg"><PlayCircle size={18}/></button><button onClick={()=>{setDoneModal(r); setIsCompleted(true);}} title="Done" className="text-green-600 hover:text-green-800 bg-green-50 p-2 rounded-lg"><CheckCircle2 size={18}/></button></td></tr>)}</tbody></table>
 }
 </Card>
